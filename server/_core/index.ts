      ...
      if (buttonId) {
        ...
        if (confirmMatch) {
          ...
          if (result.success) {
           ...
           // Este botão indica que o entregador SAIU para a entrega
           // Ação: enviar template "Pronto (Delivery)" ao cliente informando que o pedido está a caminho
           // NÃO finaliza o pedido - apenas notifica o cliente
           ...
           try {
             const { getPublicOrderByNumber, updateOrderStatus, getWhatsappConfig, getEstablishmentById, getOrderItems, getDeliveryFinisher } = await import('../db');
             ...
             if (!orderData) {
               ...
               if (finisher !== 'driver') {
                 ...
                 console.log('[WhatsApp Webhook] Pedido não está em status válido:', orderData.status);
               } else {
                 // NÃO mudar status para completed - apenas notificar o cliente
                 // O pedido continua em out_for_delivery
                 console.log('[WhatsApp Webhook] Entregador saiu para entrega, notificando cliente:', orderNumber);
                 
                 // Enviar notificação ao cliente usando template "Pronto (Delivery)" = ready
                 if (orderData.customerPhone) {
                   const config = await getWhatsappConfig(establishmentId);
                   if (config && config.instanceToken && config.status === 'connected' && config.notifyOnReady) {
                     ...
                     console.log('[WhatsApp Webhook] Template Pronto (Delivery) enviado ao cliente');
                   } else {
                     console.log('[WhatsApp Webhook] Template Pronto (Delivery) não enviado - notifyOnReady desativado ou WA desconectado');
                   }
                 }
                 ...
                 if (senderPhone) {
                   ...
                   if (config?.instanceToken) {
                     ...
                     await sendText(
                       config.instanceToken,
                       phone,
                       `✅ Entrega do pedido ${orderNumber} confirmada! O cliente foi notificado.`
                     );
                   }
                 }
               }
             ...
             console.error('[WhatsApp Webhook] Erro ao processar saída para entrega:', deliveryError);
           }
        } else if (buttonId.match(/delivery_done_(#P\d+)/)) {
           // === ENTREGADOR CLICOU "MARCAR COMO ENTREGUE" ===
           // Este botão finaliza o pedido no admin
           const doneMatch = buttonId.match(/delivery_done_(#P\d+)/);
           const orderNumber = doneMatch[1];
           ...
           try {
             ...
             if (!orderData) {
               ...
               const finisher = await getDeliveryFinisher(establishmentId);
               if (finisher !== 'driver') {
                 console.log('[WhatsApp Webhook] deliveryFinisher não é driver, ignorando botão entregue');
               } else if (orderData.status === 'completed' || orderData.status === 'cancelled') {
                 console.log('[WhatsApp Webhook] Pedido já finalizado/cancelado:', orderData.status);
                 // Informar o entregador
                 if (senderPhone) {
                   const config = await getWhatsappConfig(establishmentId);
                   if (config?.instanceToken) {
                     const phone = senderPhone.replace('@s.whatsapp.net', '').replace('@c.us', '');
                     await sendText(config.instanceToken, phone, `⚠️ Pedido ${orderNumber} já foi ${orderData.status === 'completed' ? 'finalizado' : 'cancelado'}.`);
                   }
                 }
               } else {
                 // Atualizar status para completed
                 await updateOrderStatus(orderData.id, 'completed');
                 console.log('[WhatsApp Webhook] Pedido finalizado pelo entregador (entregue):', orderNumber);
                 
                 // Enviar notificação de conclusão ao cliente (template "completed")
                 if (orderData.customerPhone) {
                   const config = await getWhatsappConfig(establishmentId);
                 ...
                 if (senderPhone) {
                   ...
                   if (config?.instanceToken) {
                     const phone = senderPhone.replace('@s.whatsapp.net', '').replace('@c.us', '');
                     await sendText(config.instanceToken, phone, `✅ Pedido ${orderNumber} marcado como entregue! Obrigado pela entrega.`);
                   }
                 }
               }
             }
           } catch (doneError) {
             console.error('[WhatsApp Webhook] Erro ao processar marcação de entregue:', doneError);
           }
        } else if (cancelMatch) {
