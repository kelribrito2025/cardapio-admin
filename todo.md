...
## Feature: Notificações push do navegador para novos pedidos
- [x] Criar BrowserNotificationManager para gerenciar permissões
- [x] Integrar com NewOrdersContext para enviar notificação quando novo pedido chegar
- [x] Adicionar botão no header para ativar/desativar notificações push (sino)
- [x] Mostrar notificação nativa do navegador mesmo com aba em segundo plano
- [x] Ícone verde (BellRing) quando ativado, cinza (Bell) quando desativado, vermelho (BellOff) quando bloqueado
- [x] Notificação só é enviada quando document.hidden (aba em segundo plano)
